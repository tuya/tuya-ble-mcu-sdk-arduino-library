...
unsigned char TuyaDataPoint::mcu_dp_raw_update(unsigned char dpid, const unsigned char value[], unsigned short len)
{
    ...
    if (stop_update_flag == TY_ENABLE)
        return TY_SUCCESS;
    //
    send_len = tuya_uart.set_ble_uart_byte(send_len, dpid);
    send_len = tuya_uart.set_ble_uart_byte(send_len, DP_TYPE_RAW);
    //
    send_len = tuya_uart.set_ble_uart_byte(send_len, len / 0x100);
    send_len = tuya_uart.set_ble_uart_byte(send_len, len % 0x100);
    //
    send_len = tuya_uart.set_ble_uart_buffer(send_len, (unsigned char *)value, len);

    tuya_uart.ble_uart_write_frame(STATE_UPLOAD_CMD, MCU_TX_VER, send_len);
...
unsigned char TuyaDataPoint::mcu_dp_bool_update(unsigned char dpid, unsigned char value)
{
    ...
    send_len = tuya_uart.set_ble_uart_byte(send_len, dpid);
    send_len = tuya_uart.set_ble_uart_byte(send_len, DP_TYPE_BOOL);
    //
    send_len = tuya_uart.set_ble_uart_byte(send_len, 0);
    send_len = tuya_uart.set_ble_uart_byte(send_len, 1);
    //
    if (value == TY_FALSE)
    {
        send_len = tuya_uart.set_ble_uart_byte(send_len, TY_FALSE);
    }
    else
    {
        send_len = tuya_uart.set_ble_uart_byte(send_len, 1);
...
unsigned char TuyaDataPoint::mcu_dp_value_update(unsigned char dpid, unsigned long value)
{
    ...
    send_len = tuya_uart.set_ble_uart_byte(send_len, dpid);
    send_len = tuya_uart.set_ble_uart_byte(send_len, DP_TYPE_VALUE);
    //
    send_len = tuya_uart.set_ble_uart_byte(send_len, 0);
    send_len = tuya_uart.set_ble_uart_byte(send_len, 4);
    //
    send_len = tuya_uart.set_ble_uart_byte(send_len, value >> 24);
    send_len = tuya_uart.set_ble_uart_byte(send_len, value >> 16);
    send_len = tuya_uart.set_ble_uart_byte(send_len, value >> 8);
    send_len = tuya_uart.set_ble_uart_byte(send_len, value & 0xff);
...
unsigned char TuyaDataPoint::mcu_dp_string_update(unsigned char dpid, const unsigned char value[], unsigned short len)
{
    ...
    //
    send_len = tuya_uart.set_ble_uart_byte(send_len, dpid);
    send_len = tuya_uart.set_ble_uart_byte(send_len, DP_TYPE_STRING);
    //
    send_len = tuya_uart.set_ble_uart_byte(send_len, len / 0x100);
    send_len = tuya_uart.set_ble_uart_byte(send_len, len % 0x100);
    //
...
unsigned char TuyaDataPoint::mcu_dp_enum_update(unsigned char dpid, unsigned char value)
{
    ...
    send_len = tuya_uart.set_ble_uart_byte(send_len, dpid);
    send_len = tuya_uart.set_ble_uart_byte(send_len, DP_TYPE_ENUM);
    //
    send_len = tuya_uart.set_ble_uart_byte(send_len, 0);
    send_len = tuya_uart.set_ble_uart_byte(send_len, 1);
    //
    send_len = tuya_uart.set_ble_uart_byte(send_len, value);
...
unsigned char TuyaDataPoint::mcu_dp_fault_update(unsigned char dpid, unsigned long value)
{
    ...
    send_len = tuya_uart.set_ble_uart_byte(send_len, dpid);
    send_len = tuya_uart.set_ble_uart_byte(send_len, DP_TYPE_BITMAP);
    //
    send_len = tuya_uart.set_ble_uart_byte(send_len, 0);
    ...
    {
        send_len = tuya_uart.set_ble_uart_byte(send_len, 2);
        send_len = tuya_uart.set_ble_uart_byte(send_len, value >> 8);
        send_len = tuya_uart.set_ble_uart_byte(send_len, value & 0xff);
    }
    else
