...
void TuyaUart::ble_protocol_init(void)
{
    rx_buf_in = (unsigned char *)ble_uart_rx_buf;
    rx_buf_out = (unsigned char *)ble_uart_rx_buf;

    stop_update_flag = TY_DISABLE;

#ifndef BLE_CONTROL_SELF_MODE
    ble_work_state = BLE_SATE_UNKNOW;
#endif
}
...
unsigned char TuyaUart::uart_receive_input(unsigned char data)
{
    ...
    else if ((rx_buf_in > rx_buf_out) && ((unsigned long)(rx_buf_in - rx_buf_out) >= sizeof(ble_uart_rx_buf)))
    {
        //UART receive buffer is full
        return TY_ERROR;
    }
    else
    {
        ...
        if (rx_buf_in >= (unsigned char *)(ble_uart_rx_buf + sizeof(ble_uart_rx_buf)))
        {
            rx_buf_in = (unsigned char *)(ble_uart_rx_buf);
        }
...
unsigned char TuyaUart::take_byte_rxbuff(void)
{
    ...
    {
        //With data
        if (rx_buf_out >= (unsigned char *)(ble_uart_rx_buf + sizeof(ble_uart_rx_buf)))
        {
...
void TuyaUart::ble_uart_write_data(unsigned char *in, unsigned short len)
{
    if ((TY_NULL == in) || (0 == len))
    {
        return;
...
void TuyaUart::ble_uart_write_frame(unsigned char fr_type, unsigned char fr_ver, unsigned short len)
{
    unsigned char check_sum = 0;

    ble_uart_tx_buf[HEAD_FIRST] = 0x55;
    ble_uart_tx_buf[HEAD_SECOND] = 0xaa;
    ble_uart_tx_buf[PROTOCOL_VERSION] = fr_ver;
    ble_uart_tx_buf[FRAME_TYPE] = fr_type;
    ble_uart_tx_buf[LENGTH_HIGH] = len >> 8;
    ble_uart_tx_buf[LENGTH_LOW] = len & 0xff;

    len += PROTOCOL_HEAD;
    check_sum = tuya_tools.get_check_sum((unsigned char *)ble_uart_tx_buf, len - 1);
    ble_uart_tx_buf[len - 1] = check_sum;

    ble_uart_write_data((unsigned char *)ble_uart_tx_buf, len);
}

unsigned short TuyaUart::set_ble_uart_byte(unsigned short dest, unsigned char byte)
{
    unsigned char *obj = (unsigned char *)ble_uart_tx_buf + DATA_START + dest;
...
unsigned short TuyaUart::set_ble_uart_buffer(unsigned short dest, const unsigned char *src, unsigned short len)
{
    unsigned char *obj = (unsigned char *)ble_uart_tx_buf + DATA_START + dest;
